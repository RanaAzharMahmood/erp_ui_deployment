...
export const usePartyForm = ({ partyId, mode }: UsePartyFormOptions): UsePartyFormReturn => {
  ...
  const [fieldErrors, setFieldErrors] = useState<PartyFieldErrors>({});
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!formData.salesTaxNumber?.trim()) {
      errors.salesTaxNumber = 'Sales tax number is required';
    }
    ...
    if (!formData.contactCnic?.trim()) {
      errors.contactCnic = 'Contact CNIC is required';
    }
