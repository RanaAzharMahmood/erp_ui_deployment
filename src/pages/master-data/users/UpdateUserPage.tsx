  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      try {
        ...
        const response = await usersApi.v1ApiUsersIdGet(Number(id));
        ...
        // Set extended company access with permissions from API
        if (user.companies) {
          // Get company permissions mapping from API
          const companyPermissionsMap = new Map<number, Record<string, string[]>>();
          if (user.companiesWithPermissions && user.companiesWithPermissions.length > 0) {
            const parsedPermissions = apiToCompanyAccess(user.companiesWithPermissions);
            parsedPermissions.forEach((cp) => {
              companyPermissionsMap.set(cp.companyId, cp.modulePermissions);
            });
          }
          ...
          const extendedAccess: ExtendedUserCompanyAccess[] = (user.companies || []).map((company) => ({
            ...
            roleName: user.roleName || 'User',
            permissions: [],
            modulePermissions: companyPermissionsMap.get(company.id!) || {},
          }));
