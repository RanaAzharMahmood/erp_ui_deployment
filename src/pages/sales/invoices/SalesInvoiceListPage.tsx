...
import { getSalesInvoicesApi, getCompaniesApi } from '../../../generated/api/client';
...
const SalesInvoiceListPage: React.FC = () => {
  ...
  const handleDownloadPdf = useCallback(async (id: string) => {
    try {
      // Fetch the full invoice (with lines) and the company details needed for the header
      const salesInvoicesApi = getSalesInvoicesApi();
      const [invoiceRes, companiesRes] = await Promise.all([
        salesInvoicesApi.getById(Number(id)),
      ...
      await downloadInvoicePdf({
        variant: 'sales',
        documentNumber: invoice.invoiceNumber || String(id),
        date: invoice.date || '',
        company: {
        ...
        party: {
          name: invoice.customer?.partyName || 'Customer',
        },
        lines,
        discountPercent,
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 2, border: '1px solid #E0E0E0', borderRadius: '12px', bgcolor: '#FFFFFF', px: 2, height: 70 }}>
        ...
        <Button
          ...
          onClick={() => exportToCsv('sales-invoices', filteredAndSortedInvoices, [
