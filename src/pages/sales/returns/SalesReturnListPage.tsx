...
const SalesReturnListPage: React.FC = () => {
  ...
  const filteredAndSortedReturns = useMemo(() => {
    const filtered = returns.filter((returnItem) => {
      ...
      const matchesCustomer = !filters.customer || returnItem.customerName === filters.customer;
  ...
  const customerNames = useMemo(() => {
    const names = new Set(returns.map((r) => r.customerName).filter(Boolean));
    return Array.from(names);
  }, [returns]);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 2, border: '1px solid #E0E0E0', borderRadius: '12px', bgcolor: '#FFFFFF', px: 2, height: 70 }}>
        ...
        <Button
          ...
          onClick={() => exportToCsv('sales-returns', filteredAndSortedReturns, [
            { header: 'Return Number', value: 'returnNumber' },
                ...
                <TableRow>
                  <TableCell colSpan={9} align="center" sx={{ py: 8 }}>
                    <Typography color="text.secondary">
                      No sales returns found. Click "Return Sales Invoice" to add one.
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
